...
class Dashboard extends CI_Controller {
	...
	public function cek_login()
	{
		...
		if($account != NULL){
			...
			header( "Location: /index.php/dashboard/main_menu?validation=Success" );
		}
		else{
			header( "Location: /index.php/dashboard?validation=Fail" );
		}
	}
	...
	function ambilMainMenu(){
		$dataMainMenu = $this->m->ambilMainMenu('menus');
		echo json_encode($dataMainMenu);
	}	
	function ambilData(){
		$menus_id = $this->input->post('menus_id');
		$dataMenu = $this->m->ambilData($menus_id);
	...
	public function dashboard()
	{
		...
		if(isset($_SESSION['username'])){
			...
			if($_SESSION['dashboard'] != 1){
				if($_SESSION['crud'] != 3){
					header( "Location: /dashboard/plantation_group?pg=".$_SESSION['code']);
				}
				else{
					header( "Location: /dashboard/wilayah?wilayah=".$_SESSION['code']);
				}
			}
		}
		else{
	...
	public function lokasi()
	{
		...
		$data["note_pic"] = $this->Noted_model->get_pic_code($_SESSION['index']);
		...
		$data["histori_lokasi_bk"] = $this->HistoriLokasi_model->get_first_activity($lokasi, $data["lokasi"]["kebun"]);
		...
		// Material
		$data["fertilization"] = $this->Fertilization_model->get_unsur_fertilization($lokasi, $data['lokasi']['kebun']);
		$data["wc_material"] = $this->WeedControl_model->get_unsur_weed_control($lokasi, $data['lokasi']['kebun']);
		$data['ppc_material'] = $this->PlantPestControl_model->get_unsur_plant_pest_control($lokasi, $data['lokasi']['kebun']);
		$data['others_material'] = $this->Fertilization_model->get_unsur_others($lokasi, $data['lokasi']['kebun']);
	  ...
	  if($data['produksi'] == NULL){
	    if($data['lokasi']['tgl_panen_rencana'] != NULL){
	      $tgl_panen = $data['lokasi']['tgl_panen_rencana'];
		      ...
		      if(substr($data['lokasi']['status'], 0, 4) != 'NSSC'){
			    ...
			    if(date('Y', strtotime($tgl_panen)) == date('Y', strtotime($data['YEAR_BASE']['nilai']))){
		...
		for ($i=1; $i <= 15 ; $i++) {
			if($i < 10){
				if($i == 3 && $data['cek_ZN03'] == 1){
					$data['total_real'] += ($ZN03_real['total']);
				}
				else{
					$data['total_real'] += ($data["element_cost_real"]['ZN0'.$i]['total']);
				}
		...
		$tgl_forcing = date('Y-m-d', strtotime($cek_tgl_forcing['tgl_forcing']));
		if($data['lokasi']['tgl_forcing_realisasi'] != NULL){
			$data['tgl_forcing'] = $data['lokasi']['tgl_forcing_realisasi'];
		}
		else{
			$data['tgl_forcing'] = $tgl_forcing;
		}
		$data["timeline_perawatan"] = array(
			'bk_st' => $this->DataMaster_model->get_bk_st($lokasi, $data["histori_lokasi_bk"]["tgl_realisasi"], $data["lokasi"]['tgl_mulai_tanam'], $data["lokasi"]["kebun"]),
			'st_perawatan' => $this->DataMaster_model->get_st_perawatan($lokasi, $data["lokasi"]['tgl_mulai_tanam'], $data["lokasi"]["tgl_mulai_rawat"], $data["lokasi"]["kebun"]),
		...
		$data['std_interval_siram'] = $this->Irrigation_model->get_interval_siram($data['wilayah']['plantation_group']);
	...
	public function detail_material(){
      ...
      $data["tgl_rencana_forcing"] = date('Y-m-d', strtotime($data["tgl_panen"]) - (152 * 86400));
		  ...
		  if($data['produksi']['real_dan_sisa_rencana_luas'] != NULL){
			...
			if(date('Y', strtotime($data["tgl_panen"])) == $data["YEAR_BASE"]['nilai']){
				$data['std_yield'] = $this->StdYield_model->get_std_yield_code(substr($data['lokasi']['status'], 0, 4).'');
				$tahun = 'T0';
			}
			else{
			...
			$this->load->view('detil_material', $data);
	...
	public function detail_all_spk(){
		session_start();
		if(isset($_SESSION['username'])){
			$lokasi = $this->input->get('lokasi');
			$data['account'] = $this->User_model->get_user_index($_SESSION['index']);
      ...
      $data["tgl_rencana_forcing"] = date('Y-m-d', strtotime($data["tgl_panen"]) - (152 * 86400));
		  ...
		  if($data['produksi']['real_dan_sisa_rencana_luas'] != NULL){
			...
			$data["detil_spk"] = $this->Lokasi_model->get_detil_spk_sbt($lokasi, $data["lokasi"]["kebun"]);

			$this->load->view('WIP_PM/include/header');
			$this->load->view('detil_all_spk', $data);
			$this->load->view('WIP_PM/include/footer');
		}
		else{
			header( "Location: /PIS" );
	...
	public function detail_activity(){
      ...
      $data["tgl_rencana_forcing"] = date('Y-m-d', strtotime($data["tgl_panen"]) - (152 * 86400));
		  ...
		  if($data['produksi']['real_dan_sisa_rencana_luas'] != NULL){
			...
			$this->load->view('WIP_PM/include/header');
			$this->load->view('detil_activity', $data);
			$this->load->view('WIP_PM/include/footer');
		}
		else{
			header( "Location: /PIS" );
