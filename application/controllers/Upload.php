...
class Upload extends CI_Controller {
  ...
  function insert_gallery(){
    ...
    header( "Location: /PIS".$url );
  }

  function insert_peta(){
    session_start();
		$this->User_model->reload_history_login($_SESSION['index']);
    $lokasi = $this->input->post('lokasi');
    $caption = $this->input->post('caption');
    ...
    $ekstensi_diperbolehkan	= array('png','jpg','jpeg');
    $nama_foto = $lokasi.'_peta_'.date('dmY_His');
    $x_foto = explode('.', $_FILES['foto']['name']);
    $ekstensi_foto = strtolower(end($x_foto));
    $ukuran_foto	= $_FILES['foto']['size'];
    $file_tmp_foto = $_FILES['foto']['tmp_name'];
    ...
    if($_FILES['foto'] != NULL){
      if(in_array($ekstensi_foto, $ekstensi_diperbolehkan) == true){
        if($ukuran_foto < 20480000){
          move_uploaded_file($file_tmp_foto, 'assets/images/peta/'.$nama_foto.'.'.$ekstensi_foto);
          $url_foto = '/PIS/assets/images/peta/'.$nama_foto.'.'.$ekstensi_foto;
          //echo 'Berhasil Diubah';
        }
        else{
          $url_foto = NULL;
    ...
    $this->GalleryLokasi_model->set_peta_lokasi($lokasi, $caption, $url_foto);
    header( "Location: /PIS".$url );
  }
  ...
  function delete_peta(){
    ...
    $this->GalleryLokasi_model->delete_peta_lokasi($id);
    header( "Location: /PIS".$url );
  }
}
