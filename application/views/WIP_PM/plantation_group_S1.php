...
<?php
  ...
  $pp_good = "";
  $pp_poor = "";
  foreach ($performance_pg as $pp) {
    $pp_label .= "'".$pp->wilayah."', ";
    $pp_excellent .= round(($pp->Excellent / $pp->Total) * 100, 2).', ';
    $pp_good .= round(($pp->Good / $pp->Total) * 100, 2).', ';
    $pp_poor .= round(($pp->Poor / $pp->Total) * 100, 2).', ';
  }
?>
<script>
  var config_category_ha = {
    type: 'bar',
    data: {
      labels: [<?php echo $pp_label; ?>],
			datasets: [{
				label: 'Excellent',
				backgroundColor: '#32CD32',
				stack: 'Stack 0',
				data: [
					<?php echo $pp_excellent; ?>
				]
			}, {
				label: 'Good',
