...
class HistoryGallery_model extends CI_Model
{
	...
	function get_history_gallery(){
		$query = $this->db->query("SELECT
																  wilayah.`plantation_group`,
																  wilayah.`code`,
																  lokasi.`lokasi`,
																  lokasi_history_gallery.`lokasi` AS info
																FROM
																  wilayah,
																  lokasi
																  LEFT JOIN lokasi_history_gallery
																  ON lokasi.`lokasi` = (lokasi_history_gallery.`lokasi`)
																WHERE wilayah.`code` = SUBSTRING(lokasi.`kebun`, 1, 3)
																GROUP BY lokasi.`lokasi`");
