...
class Noted_model extends CI_Model
{
	...
	function get_note_gallery($lokasi){
		$query = $this->db->query("SELECT
																  *
																FROM
																  note
																WHERE note.`lokasi` = '$lokasi'
																ORDER BY note.`tgl_start` DESC");

		return $query->result();
	}
	function get_note_comment_gallery($id){
		$query = $this->db->query("SELECT
																  *
																FROM
																  note_comment
																WHERE note_comment.`id_note` = '$id'
																ORDER BY note_comment.`tgl_start` DESC");
